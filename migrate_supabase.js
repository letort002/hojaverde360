...
fs.writeFileSync("src/supabase.js", supabaseClient, "utf8");
console.log("✅ src/supabase.js creado");

// ── 2. Reescribir HV_Mensajeria.jsx con Supabase ─────────────
const newMensajeria = `import { useState, useEffect, useRef } from "react";
...
export default function HVMensajeria() {
  ...
  async function exportExcel() {
    ...
    const rows=tasks.map(t=>({ "ID":t.id,"Fecha":t.fecha,"Hora":t.hora,"Hora Fin":t.hora_fin||"—","Tipo":typeLabels[t.tipo],"Descripción":t.descripcion,"Destino":t.destino,"Mensajero":messengers[t.messenger]?.nombre,"Prioridad":t.prioridad,"Estado":t.status.replace("-"," ").replace(/\b\w/g,l=>l.toUpperCase()),"Notas":t.nota||"","Obs. Entrega":t.firma_obs||"","Motivo Rechazo":t.motivo_rechazo||"" }));
...
fs.writeFileSync("src/HV_Mensajeria.jsx", newMensajeria, "utf8");
console.log("✅ src/HV_Mensajeria.jsx migrado a Supabase");
console.log("✅ src/supabase.js creado");
console.log("\nEjecuta: npm run dev");
